...
So this project hands the interface back to the official product and **spends all engineering effort outside the window: residency and long tasks, the Agent's execution environment, security hardening, connection and updates.** The benefits of this approach hold on their own:
...
- **Smart mode reuses the instance you are already running.** It probes in order: the official instance on `127.0.0.1:3080` → a `dsh` on PATH → an npx-cached package → the bundled runtime. Browser, CLI, and desktop then share one live Harness process, with sessions synced in real time.
