...
- **内置插件接缝**：客户端运行时闭包可携带插件（当前是插件市场），并按官方 in-box bundle 的方式接入——`resolveBundleDir()` 解析 `dsh.profile.bundles` 时先查 dsh 安装目录再查 profile，而 `dsh plugin` 明确不碰非依赖项的 bundle。因此接入只需两步：在 `<DSH_HOME>/profiles/node_modules` 建一条指向闭包的软链（harness 每次启动的 healing 只增不删，这条链得以共存），以及把包名写进 profile 的 `dsh.profile.bundles`。两步都可一步撤销，这正是它敢自动执行的前提：插件在加载期抛错会让整棵插件树启动失败。只在即将启动**内置**运行时时接入（插件的 `@deepseek-ai/*` 从闭包向上解析，交给用户自装的 dsh 会产生第二份 Service 实例），其余来源改为撤回；首次接入后未就绪则撤回重试；用户自行作为 profile 依赖安装过则完全不介入。见 `src/main/bundled-plugin.ts`。
