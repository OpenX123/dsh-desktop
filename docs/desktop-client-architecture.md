...
- **Electron main** (`src/main/`): owns the windows and Web UI runtime. In **local mode** it probes the default loopback instance and otherwise spawns `dsh web --port 0`; in **connect mode** it uses a configured Web UI origin. The command resolves from `DSH_DESKTOP_DSH`, the optional app-bundled `@deepseek-ai/dsh`, development sibling checkouts, then PATH. Child supervision provides bounded relaunch, per-generation readiness, stale-callback rejection, and graceful shutdown (SIGTERM→SIGKILL on POSIX; `taskkill /T /F` on Windows). Packaged builds run the bundled CLI through Electron's Node mode (`ELECTRON_RUN_AS_NODE`).
