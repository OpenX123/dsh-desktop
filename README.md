...
| ③ 原装直连（本项目） | 窗口直接加载官方 Web UI，工程投入放在连接、安全、执行环境与桌面体验上 | 不做自研界面，桌面增强以官方界面能承载的为界 |

**Q：对用户来说，主要价值是什么？**

1. **装好就能用**：安装包内置官方运行时，不用装 Node.js、pnpm 或 `dsh` CLI，配置一个 API Key 即可对话；托盘常驻，关闭窗口不退出应用；
2. **复用你正在跑的 dsh**：智能模式依次探测运行中的官方实例、PATH 上的 `dsh`、npx 缓存包——终端、浏览器与桌面端共享一个 Harness 进程与 `~/.dsh`，会话实时同步；固定地址模式可直连你自行维护的 Web UI 地址；
3. **原汁原味的官方体验**：窗口就是官方 Web UI，官方文档、教程与快捷键全部对得上；官方发版后，智能模式与固定地址当天即可用上，内置运行时随应用内更新跟进。

**Q：和直接用浏览器访问 Web UI 有什么区别？除了不用装 Node.js 之外**

官方浏览器用法是装好 Node.js 后执行 `npx @deepseek-ai/dsh web`，再手动打开它给出的地址——关掉这个终端，服务就停了。客户端把这件事工程化：安装包自带运行时，双击即用；关闭窗口不退出应用，托盘常驻随时唤回；智能模式先探测你已在跑的实例，没有时依次用 PATH 上的 `dsh`、npx 缓存包在后台启动，兜底才用内置运行时；另有浏览器没有的应用内更新、连接来源与版本透明展示。
...
官方 Web UI 自身的能力（技能、插件、交互等）不需要等本项目的排期，会随官方发版直接出现在窗口里。桌面外壳自身的后续工作见[当前状态](#当前状态)与 [TODO](TODO.md)：macOS/Windows 签名与公证、系统通知、OS Keychain、语音输入，以及内置运行时的独立更新通道、运行中实例的周期探测提示等。
