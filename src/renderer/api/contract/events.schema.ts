/**
 * events domain zod schemas of the public /api wire contract (vendored,
 * self-contained): MuxFrame / HostFrame unions (discriminatedUnion('type')).
 * A frame is the payload slot of the ServerRequest full form.
 * @module desktop/renderer/api/contract/events.schema
 */
...
import { contentBlockSchema, messageIdSchema, sessionEventSchema, sessionIdSchema, toolEventViewSchema } from './sessions.schema.ts'
import { taskViewSchema } from './tasks.schema.ts'
import { workspaceIdSchema, workspaceViewSchema } from './workspace.schema.ts'

/** Question fields validated strictly against the wire vocabulary. */
...
export const hostFrameSchema = z.discriminatedUnion('type', [
  ...
  // args stays wide, the same posture as session/projection's value.
