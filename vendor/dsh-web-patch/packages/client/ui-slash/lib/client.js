	...
	factory: (require) => {
		...
		*/
		var SlashController = class {
			...
			/**
			* What the programmatic launcher opened, or null for trigger-detected or
			* closed menus: the source name for a single-source launch
			* ({@link toggleSource}), or the trigger char for a full-roster launch
			* ({@link toggleTrigger}). Composer chrome subscribes to this store for
			* the launcher's expanded state without owning a second menu model.
			*/
			...
			*/
			...
			/**
			* Toggle a menu containing every source registered for one trigger — the
			* composer launcher's full-roster open, identical to what typing the
			* trigger produces. The launcher store records the trigger char (rather
			* than a source name) so chrome can distinguish the full-roster open from
			* a single-source {@link toggleSource} launch. Picks reuse the ordinary
			* source callbacks and scoped input mutation pipeline unchanged.
			* @param trigger - trigger char whose roster opens.
			* @param hit - synthetic hit carrying position and pick-time draft CAS.
			*/
			toggleTrigger(trigger, hit) {
				if (this.disposed) return;
				if (this.launcher.getSnapshot() === trigger && this.menu.getSnapshot().open) {
					this.dismiss();
					return;
				}
				...
				this.stopFetch();
				this.hit = hit;
				this.launcher.set(trigger);
				this.menu.set(seedGroups(this.menu.getSnapshot(), roster.map((source) => source.name)));
				this.reduce({
					type: "hit",
					hit
				});
		...
		*/
		...
		var SlashService = class extends _deepseek_ai_cordis.Service {
		...
		//#endregion
		//#region \0dsh-css:/Users/bruce/Documents/vscode-projects/test-bruc3van/packages/client/ui-slash/src/client/MenuView.module.css.mjs
		const css = ".ReSEFq_menu{z-index:100;--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);min-width:min(260px,100%);max-width:min(537px,100%);max-height:320px;box-shadow:var(--dsw-shadow-lv3);border-radius:12px;flex-direction:column;padding:4px;display:flex;position:absolute;bottom:calc(100% + 4px);left:0;overflow:hidden}.ReSEFq_viewport{flex-direction:column;min-height:0;display:flex;overflow-y:auto}.ReSEFq_item{cursor:pointer;width:100%;min-height:40px;color:var(--dsw-alias-label-primary);text-align:left;background:0 0;border:none;border-radius:10px;align-items:center;gap:8px;padding:8px 10px;font-size:14px;line-height:22px;display:flex}.ReSEFq_item:hover,.ReSEFq_item.ReSEFq_active{background:var(--dsw-alias-interactive-bg-hover)}.ReSEFq_itemIcon{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);flex:none;justify-content:center;align-items:center;display:inline-flex}.ReSEFq_itemName{text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}.ReSEFq_itemDescription{text-overflow:ellipsis;white-space:nowrap;min-width:0;color:var(--dsw-alias-label-tertiary);flex:1;overflow:hidden}.ReSEFq_groupTitle{color:var(--dsw-alias-label-tertiary);padding:8px 10px;font-size:12px;line-height:16px}.ReSEFq_loading{min-height:40px;color:var(--dsw-alias-label-dimmed);align-items:center;padding:8px 10px;font-size:14px;line-height:22px;display:flex}";
		...
		var MenuView_module_css_default = {
			"itemDescription": "ReSEFq_itemDescription",
			"viewport": "ReSEFq_viewport",
			"loading": "ReSEFq_loading",
			"item": "ReSEFq_item",
			"groupTitle": "ReSEFq_groupTitle",
			"menu": "ReSEFq_menu",
			"itemIcon": "ReSEFq_itemIcon",
			"active": "ReSEFq_active",
			"itemName": "ReSEFq_itemName"
		};
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => {
				return ctx.locale.register(MENU_NS, {
					zh,
					en
				});
