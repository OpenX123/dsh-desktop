...
//#region lib/types/skill-state.js
/**
* Durable per-user skill management state (`$DSH_HOME/skills-state.json`):
* the set of skill names the user disabled through the settings surface.
*
* Kept out of the settings service on purpose: a disabled skill is a
* catalog-admission policy this proxy applies on every `skill.list` read,
* not a preference a configuration surface edits through the settings
* document. The file is private to this module — the management RPCs are the
* only access path, so the wire and the store can never drift.
*
* Writes are atomic (temp file + rename) and serialized through one promise
* chain; concurrent toggles converge on the last-writer value without
* interleaving partial documents.
* @module dsh-host-apiproxy/skill-state
*/
/** One parseable document, or null when the file is absent or unreadable. */
function parseDocument(raw) {
	try {
		...
		if (!Array.isArray(disabled) || disabled.some((name) => typeof name !== "string" || name.length === 0)) return null;
		return { disabled: [...new Set(disabled)].sort() };
	} catch {
		return null;
	}
}
/** The durable disabled-skill set. */
var SkillStateStore = class {
	/** Null = not loaded yet; loads once, then serves the live set. */
	disabled = null;
	/** Write serialization chain; always a settled-recovery promise, never rejects. */
	writes = Promise.resolve();
	file;
	/** @param path - state file path; defaults to `$DSH_HOME/skills-state.json`. */
	constructor(path = dshHomePath("skills-state.json")) {
		this.file = path;
	}
	/**
	* The currently disabled names.
	* @returns the live set (a fresh readonly snapshot per caller).
	*/
	async snapshot() {
		if (this.disabled === null) await this.load();
		/* v8 ignore next -- load() sets disabled before resolving. */
		return new Set(this.disabled ?? []);
	}
	/** Whether one skill name is disabled. */
	async isDisabled(name) {
		return (await this.snapshot()).has(name);
	}
	/**
	* Disable or re-enable one skill; persists before resolving so the next
	* catalog read observes the change.
	* @param name - skill name.
	* @param enabled - false hides the skill from every catalog.
	*/
	async setEnabled(name, enabled) {
		const snapshot = await this.snapshot();
		if (!(enabled ? snapshot.has(name) : !snapshot.has(name))) return;
	...
	/** Drop one name from the disabled set (e.g. after deleting the skill). */
	async forget(name) {
		const snapshot = await this.snapshot();
		if (!snapshot.has(name)) return;
		const next = new Set(snapshot);
		next.delete(name);
		this.disabled = next;
		await this.persist();
	...
	async load() {
		let document = null;
		try {
			document = parseDocument(await readFile(this.file, "utf8"));
		} catch {}
		this.disabled = new Set(document?.disabled ?? []);
	}
	async persist() {
		const document = { disabled: [...this.disabled ?? []].sort() };
...
const sessionSummarySchema = z$1.object({
	...
	projections: z$1.lazy(() => {
		return sessionProjectionsBlockSchema;
	}).optional()
});
...
/** Skill sources that resolve to user-owned files on disk (deletable). */
const DELETABLE_SKILL_SOURCES = new Set([
	"user-dsh",
	"user-agents",
	"custom",
	"project-dsh",
...
/**
* The disk target a skill deletion removes, or undefined when the skill is
* not a locally stored one. A directory-bundle skill (body at `SKILL.md`)
* removes its whole directory; a flat Markdown skill removes the file. The
* path must sit under a `skills` directory segment — defense in depth so a
* misbehaving provider can never name an arbitrary path for deletion.
* @param definition - the loaded skill definition.
* @returns the file or directory to remove.
*/
function deletableSkillPath(definition) {
	if (!DELETABLE_SKILL_SOURCES.has(definition.source)) return void 0;
	const path = definition.path;
	if (path === void 0 || !path.split(sep).includes("skills")) return void 0;
...
function createApiProxy(ctx, defaults) {
	...
	*/
	...
	/** Durable disabled-skill set (management RPCs are its only writers). */
	const skillState = new SkillStateStore();
	/**
	* The registry view scope of the skills management surface: the default
	* preset's standing layer (its skill-local provider discovers the user,
	* custom, and bundled roots). Falls back to the global layer when no
	* preset roster exists.
	* @returns the scope to read the management catalog through.
	*/
	async function adminSkillsScope() {
		const presets = ctx.get("agentPresets");
		if (presets === void 0) return void 0;
		try {
			return await presets.standingKeyFor(presets.defaultId);
		} catch {
			return;
		}
	}
	/** Publish a skills/change notification (forwarded to every client). */
	function notifySkillsChange() {
		for (const callback of ctx.events.dispatch("emit", ["skills/change"])) try {
			const returned = callback();
			Promise.resolve(returned).catch((error) => {
				ctx.logger.warn(`skills/change listener rejected: ${String(error)}`);
			});
		} catch (error) {
			ctx.logger.warn(`skills/change listener threw: ${String(error)}`);
		}
	}
	...
	*/
	...
	function ensureWorkspace(path) {
		const operation = workspaceCreationChain.then(async () => {
			...
			return {
				workspace: await ctx.workspace.create(path),
				created: true
			};
		});
	...
	return {
		...
		skills: {
			async list(request) {
				...
				try {
					const disabled = await skillState.snapshot();
					return ok(request, { skills: (await skillRegistry.list({
						cwd,
						scope
					})).filter(isUserInvocable).filter((skill) => !disabled.has(skill.name)).map((skill) => ({
						name: skill.name,
			...
			async adminList(request, signal) {
				const skillRegistry = ctx.get("skills");
				if (skillRegistry === void 0) return err(request, {
					code: "internal",
					message: "skill registry is absent: the host composition mounts no @deepseek-ai/dsh-skill",
					details: {}
				});
				const scope = await adminSkillsScope();
				try {
					const disabled = await skillState.snapshot();
					...
					for (const skill of skills) {
						let deletable = { kind: "no" };
						if (DELETABLE_SKILL_SOURCES.has(skill.source)) try {
							const definition = await skillRegistry.get(skill.name, {
								scope,
								signal
							});
							const target = definition === void 0 ? void 0 : deletableSkillPath(definition);
							if (target !== void 0) deletable = {
								kind: "local",
								path: target
							};
						} catch {}
					...
					return ok(request, { skills: rows });
				} catch (error) {
					return err(request, {
						code: "internal",
						message: `skill management listing failed: ${String(error)}`,
						details: {}
					});
				}
			},
			async setEnabled(request) {
				...
				if (!isSkillName(name)) return err(request, {
					code: "skill-invalid",
					message: `invalid skill name "${name}"`,
					details: { name }
				});
				try {
					await skillState.setEnabled(name, enabled);
				} catch (error) {
					return err(request, {
						code: "internal",
						message: `saving skill state failed: ${String(error)}`,
						details: {}
					});
				}
				notifySkillsChange();
			...
			async remove(request) {
				...
				let definition;
				try {
					definition = await skillRegistry.get(name, { scope });
				} catch (error) {
					return err(request, {
						code: "internal",
						message: `skill lookup failed: ${String(error)}`,
						details: {}
					});
				}
				if (definition === void 0) return err(request, {
					code: "skill-not-found",
					message: `skill "${name}" not found`,
					details: { name }
				});
				const target = deletableSkillPath(definition);
				if (target === void 0) return err(request, {
					code: "skill-not-deletable",
					message: `skill "${name}" is not a locally stored skill and cannot be deleted`,
					details: { name }
				});
				try {
					...
					return err(request, {
						code: "internal",
						message: `skill deletion failed: ${String(error)}`,
						details: {}
					});
				}
				await skillState.forget(name);
				notifySkillsChange();
				return ok(request, {});
			}
		},
		...
		events: {
			mux(_request, signal) {
				...
				if (tasks !== void 0) for (const session of ctx.sessions.list()) {
				...
				const disposers = [
					...
					ctx.on("session/created", (session) => {
						...
						const views = tasks === void 0 ? [] : taskViews(tasks.list(ctx.agents.get(session.id)));
						if (views.length > 0) queue.push(frame({
							type: "session/tasks",
							sessionId: session.id,
							tasks: views
					...
					...tasks === void 0 ? [] : [tasks.onTasksChanged((owner) => {
						if (owner !== void 0) {
							queue.push(frame({
								type: "session/tasks",
								sessionId: owner.id,
								tasks: taskViews(tasks.list(owner))
							}));
							return;
						}
...
* skillListValueSchema, skillAdminListRequestSchema / skillAdminListValueSchema,
* skillSetEnabledRequestSchema / skillSetEnabledValueSchema, skillRemoveRequestSchema /
...
/** skill.adminList request payload (empty, like host.describe). */
const skillAdminListRequestSchema = z$1.object({});
/** Deletable arm of one management row (a local path, or not deletable). */
const skillDeletableSchema = z$1.discriminatedUnion("kind", [z$1.object({
	kind: z$1.literal("local"),
	path: z$1.string()
}), z$1.object({ kind: z$1.literal("no") })]);
/** skill.adminList response row. */
const skillAdminEntrySchema = skillEntrySchema.extend({
	source: z$1.string(),
	provider: z$1.string(),
	disabled: z$1.boolean(),
	deletable: skillDeletableSchema
});
/** skill.adminList response value. */
const skillAdminListValueSchema = z$1.object({ skills: z$1.array(skillAdminEntrySchema) });
/** skill.setEnabled request payload. */
const skillSetEnabledRequestSchema = z$1.object({
	name: z$1.string().min(1),
	enabled: z$1.boolean()
});
/** skill.setEnabled response value. */
const skillSetEnabledValueSchema = z$1.object({});
/** skill.remove request payload. */
const skillRemoveRequestSchema = z$1.object({ name: z$1.string().min(1) });
/** skill.remove response value. */
const skillRemoveValueSchema = z$1.object({});
//#endregion
...
const UNARY_ROUTES = {
	...
	"skill.adminList": {
		schema: skillAdminListRequestSchema,
		invoke: (api, r, signal) => api.skills.adminList(r, signal)
	},
	"skill.setEnabled": {
		schema: skillSetEnabledRequestSchema,
		invoke: (api, r) => api.skills.setEnabled(r)
	},
	"skill.remove": {
		schema: skillRemoveRequestSchema,
		invoke: (api, r) => api.skills.remove(r)
	},
	"agentPreset.list": {
...
//#endregion
//#region lib/types/api/tasks.schema.js
/**
...
const muxFrameSchema = z$1.discriminatedUnion("type", [
	...
	z$1.object({
		type: z$1.literal("session/tasks"),
		sessionId: sessionIdSchema,
		tasks: z$1.array(taskViewSchema)
	}),
	z$1.object({
...
const UNARY_VALUE_SCHEMAS = {
	...
	"skill.adminList": skillAdminListValueSchema,
...
var AbstractApiClient = class {
	...
	*/
	...
	sessions = {
		...
		models: (payload, signal) => {
			return this.callUnary("session.models", payload, signal);
		},
	...
	skills = {
		...
		adminList: (payload, signal) => this.callUnary("skill.adminList", payload, signal),
